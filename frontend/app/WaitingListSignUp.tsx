...
export default function WaitingListSignUp() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setStatus('Error adding toW waitlist. Please try again.');
    }
  }
  ...
  return (
    <div className="font-sans grid grid-row items-center justify-items-center min-h-screen p-8 pb-20 gap-2 sm:p-20">
      ...
      <form onSubmit={handleSubmit}>
          <div className="w-[400px] items-center justify-items-center grid grid-cols-1 gap-2 mt-2">  
            <Input 
              name="email" 
              type="email" 
              ...
              required
              className={`w-full max-w-md px-4 py-2 h-10 border rounded-md 
                            ${errorStatus ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-gray-[#021b03]'}`} 
              />
              <div className="w-full max-w-md text-center mt-3">
            ...
            <Button type="submit" className="w-full max-w-md h-10 bg-radial from-[#021b03] via-[#001202] to-[#000201] text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
              Join Now
            </Button>
          </div>
      </form>
